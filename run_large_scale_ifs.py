...
from src.common.data_reader_ifs import (  # noqa: E402
...
class ModelAdapter:
    ...
    """Return ``(V, H, W)`` ground truth (ERA5 0.25° for metrics; adapter.lat may differ for NC save)."""
    ...
    *,
    ...
    for i, name in enumerate(channel_names):
        ...
        if path is None or not path.exists():
            continue
        out[i] = load_npy_2d(path, flip_north_south=False).astype(np.float32)
    return out
...
def build_adapter(
    ...
    if model == "graphcast":
        ...
        return ModelAdapter(
            "graphcast",
            lambda it, leads, **_unused: gc_runner.run(it, leads),
            names,
            _lat_721(),
            _gc_gt,
        )
    if model == "graphcast_operational":
        ...
        gc_runner = GraphcastOperationalForecastRunner(era5_root=era5_root)

        def _gc_oper_gt(t: datetime) -> np.ndarray:
            return load_snapshot_by_channel_names(
                t, names, root=era5_root, flip_north_south=False
            )
    ...
    if model == "aurora":
        from src.models.aurora_runner_ifs import aurora_channel_names, run_aurora_forecast
    ...
    if model == "neuralgcm":
        from src.models.neuralgcm_runner import neuralgcm_channel_names, run_neuralgcm_forecast

        names = neuralgcm_channel_names()
        ...
        return ModelAdapter(
            ...
            names,
            np.linspace(90.0, -90.0, 256, dtype=np.float64),
            _ngt,
        )
    if model == "aifs":
        return ModelAdapter(
...
def _expected_nc_path_for_init(
    init_time: datetime,
    lead_times: List[int],
    save_lead_range: tuple[int, int] | None,
    nc_root: Path,
) -> Path | None:
    """Return the representative NC path for an init (max lead inside save range)."""
    if save_lead_range is None:
        return None
    lo, hi = save_lead_range
    in_range = [int(x) for x in lead_times if lo <= int(x) <= hi]
    if not in_range:
        return None
    lead = max(in_range)
    init_str = init_time.strftime("%Y%m%d%H")
    date_part = init_time.strftime("%Y-%m%d")
    return nc_root / init_str / f"{date_part}-{lead:02d}.nc"
...
def _normalize_fuxi_save_name(name: str) -> str:
    """
    Normalize save_vars aliases for FuXi channel names.
    FuXi uses `r_<level>` (relative humidity), while some shared command
    templates pass `q_<level>`.
    """
    norm = _normalize_fuxi_eval_name(name)
    if norm.startswith("q_"):
        return "r_" + norm.split("_", 1)[1]
    return norm
...
def _fuxi_drop_rh_eval_channels(
    ...
    """
    FuXi pressure moisture is ``r_*`` (RH). ERA5 analysis stacks used here follow
    the Pangu-style humidity channel (``q_*`` when present); RH is intentionally
    omitted from skill scores, and loading ``r_*`` truth often fails — so metrics
    exclude all ``r_*`` channels.
    """
    out_idx: List[int] = []
    out_names: List[str] = []
...
def main() -> None:
    ...
    if args.model == "fuxi" and save_vars:
        save_vars = [_normalize_fuxi_save_name(x) for x in save_vars]

    for init_time in inits:
        expected_nc = _expected_nc_path_for_init(init_time, lead_times, args.save_lead_range, nc_root)
        if not sufficient_era5_snapshot(args.model, init_time, layout):
            if expected_nc is not None and expected_nc.exists():
                logger.info(
                    "Skip init %s (missing NPY, but existing forecast found: %s).",
                    init_time,
                    expected_nc,
                )
            else:
        ...
        try:
            ...
            if expected_nc is not None and expected_nc.exists():
                logger.info(
                    "Skip init %s (%s), existing forecast found: %s",
                    init_time,
                    e,
                    expected_nc,
                )
        ...
        for lead in lead_times:
            ...
            if args.mode in ("offline", "both"):
                ...
                if lo <= lead <= hi:
                    sub, sub_names = subset_channels(pred, adapter.channel_names, save_vars)
                    lat = adapter.lat
