...
try:
    ...
    # Local fallback for standalone use without earth2studio package.
    def batch_coords():
        def _decorator(fn):
            return fn
    ...
    def handshake_coords(*_args, **_kwargs):
        return None

    def handshake_dim(*_args, **_kwargs):
        return None

    def OptionalDependencyFailure(*_args, **_kwargs):
        return None
...
try:
    ...
    # GPU extras are optional for CPU-based Vitart tracking.
    cp = None
...
class _TCTrackerBase:
    ...
    def get_local_max(
        ...
        """
        if x.is_cuda and cp is not None and cucim_peak_local_max is not None:
            x_ = cp.from_dlpack(x)
