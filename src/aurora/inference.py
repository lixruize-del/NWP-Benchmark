...
# --- Configuration ---
DATA_DIR = BASE_DIR / "assets" / "data" / "era5_aurora"
...
def load_data(target_date: str, device: str) -> Batch:
    ...
    batch = Batch(
        ...
        atmos_vars={
            "t": torch.from_numpy(upper_ds["t"].values[:2][None]).float().to(device),
            "u": torch.from_numpy(upper_ds["u"].values[:2][None]).float().to(device),
            "v": torch.from_numpy(upper_ds["v"].values[:2][None]).float().to(device),
            "q": torch.from_numpy(upper_ds["q"].values[:2][None]).float().to(device),
            "z": torch.from_numpy(upper_ds["z"].values[:2][None]).float().to(device),
        },
        metadata=Metadata(
            ...
            atmos_levels=tuple(int(level) for level in upper_ds.pressure_level.values),
        ),
    )
    return batch
